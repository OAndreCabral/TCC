...
const EditarEncomenda = () => {
    ...
    const [encomendaRecord, setEncomendaRecord] = useState([]);
    ...
    useEffect(() => {
        ...
        }).then(async responseRecord => {
            setEncomendaRecord(responseRecord.data);
            form.setFieldsValue({
                filial: responseRecord.data.branchId,
                fornecedor: responseRecord.data.providerId,
                date: moment(responseRecord.data.date),
                status: responseRecord.data.status,
                description: responseRecord.data.description,
            })

            handleFornecedorChange(responseRecord.data.providerId)
        }).catch(error => {
            console.error('Erro ao buscar filiais:', error);
        });
    ...
    const onFinish = async (values) => {
        ...
        try {
            const response = await axios.put(`http://localhost:3000/schedules/${id}`, formData, {
                headers: {
        ...
        <Content className={Styles.content}>
            ...
            >
                <div style={{ display: 'flex', width: '100%', gap: '100px', justifyContent: 'center' }}>
                    <Form.Item style={{ width: '40%', height: '100%' }} name="nfe">
                        <Dragger
                            maxCount={1}
                            multiple={false}
